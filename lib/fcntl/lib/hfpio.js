	...
	hfpio.read(fd, read_buf, MTU_SIZE, function(bytes) {
		if (bytes > 0) {
			...
			if (r.written > 0) {
				...
				/*
				hfpio.sndPcmWait(speaker);
				if (hfpio.sndPcmAvailUpdate(speaker) > 0) {
					hfpio.writeToSpeaker(speaker, playback_buf, r.written/2, function(frames) {
						if (frames == -32) {
							console.log('  GOT HERE audioPlayback - frames == -32');
						}
						onComplete();
					});
				} else {
